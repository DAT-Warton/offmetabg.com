...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_inquiry'])) {
    ...
    if (empty($subject) || empty($category) || empty($message_text)) {
        ...
        $error = __('inquiry.name_required');
    } elseif (!$is_logged_in && empty($email)) {
        $error = __('inquiry.email_required');
    } elseif (!$is_logged_in && !filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $error = __('inquiry.email_invalid');
    } else {
        // Create new inquiry
        ...
        if ($is_logged_in) {
            // Logged in user
            $inquiryData['user_id'] = $_SESSION['customer_id'] ?? $_SESSION['admin_user'];
            $inquiryData['username'] = $_SESSION['customer_user'] ?? $_SESSION['admin_user'];
        } else {
            // Anonymous user
            $inquiryData['username'] = $name;
            $inquiryData['email'] = $email;
            $inquiryData['phone'] = $phone;
            $inquiryData['instagram'] = $instagram;
        }
        
        save_inquiry_data($inquiryData);
...
<body data-theme="<?php echo htmlspecialchars(db_get_option('active_theme', 'default')); ?>">
    <header>
        <div class="header-container">
            ...
            <div class="nav-buttons">
                <a href="/"class="btn btn-secondary">← Към началната страница</a>
    ...
    <div class="container">
            ...
            <form method="POST">
                <?php if (!$is_logged_in): ?>
                    ...
                    <div class="form-group">
                        <label for="name"><?php echo __('inquiry.name_placeholder'); ?> *</label>
                        <input type="text"id="name"name="name"required placeholder="<?php echo __('inquiry.name_placeholder'); ?>">
                    </div>
                    
                    <div class="form-group">
                        <label for="email"><?php echo __('inquiry.email'); ?> *</label>
                        <input type="email"id="email"name="email"required placeholder="example@email.com">
                    </div>
                    
                    <div class="contact-options">
                        <p class="contact-info"><?php echo __('inquiry.additional_contact'); ?></p>
                        
                        <div class="form-group">
                            <label for="phone"><?php echo __('inquiry.phone'); ?></label>
                            <input type="text"id="phone"name="phone"placeholder="+359 ... / Viber / WhatsApp">
                        </div>
                ...
                <div class="form-group">
                    ...
                    <textarea id="message"name="message"required placeholder="<?php echo __('inquiry.message'); ?>..."rows="6"></textarea>
                </div>
        ...
        <div class="info-section">
            <p><?php echo __('inquiry.login_to_track'); ?></p>
            <a href="auth.php?action=login&redirect=inquiries.php"class="btn btn-secondary"><?php echo __('login'); ?></a>
        </div>
        <?php endif; ?>
    </div>
