<?php
/**
 * Email System using Local SMTP (Postfix)
 * 
 * Self-hosted email without external APIs
 * Uses PHP mail() function with local Postfix SMTP server
 */
...
class EmailSender {
    ...
    public function send($to_email, $to_name, $subject, $html_body, $text_body = '') {
        ...
        if (!empty($this->config['reply_to_email'])) {
            $reply_name = $this->config['reply_to_name'];
            $reply_email = $this->config['reply_to_email'];
        ...
        // Prepare body with plain text and HTML
        $body = "--{$boundary}\r\n";
        
        // Plain text version
        $plain_text = !empty($text_body) ? $text_body : strip_tags($html_body);
