...
if (strpos($path, 'blog/') === 0) {
    $slug = substr($path, 5);
    // URL decode the slug to handle Cyrillic and special characters
    $slug = urldecode($slug);
    $post = get_post($slug);
...
if (strpos($path, 'product/') === 0) {
    ...
    // URL decode the slug to handle Cyrillic and special characters
    $productSlug = urldecode($productSlug);
    ...
    ob_start();
    include CMS_ROOT . '/templates/404.php';
    echo ob_get_clean();
    exit;
}
...
<head>
    ...
    <title>Page Not Found - <?php echo htmlspecialchars(get_option('site_title', 'OffMeta') ?? 'OffMeta'); ?></title>
    <script>
