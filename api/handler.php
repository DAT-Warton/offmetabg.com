    ...
    switch ($action) {
        ...
        case 'get_active_popup':
            ...
            foreach ($promotions as $promo) {
                if (($promo['type'] ?? '') !== 'popup') continue;
                if (!($promo['active'] ?? false)) continue;
                
                $start_ok = empty($promo['start_date']) || strtotime($promo['start_date']) <= $now;
                $end_ok = empty($promo['end_date']) || strtotime($promo['end_date']) >= $now;
                
                if ($start_ok && $end_ok) {
                    $active_popup = $promo;
                    break; // Return first matching popup
                }
            }
            
            $response = [
                'success' => true,
                'promotion' => $active_popup
            ];
            break;
