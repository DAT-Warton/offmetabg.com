...
$status_labels = [
    'pending' => ['label' => __('inquiry.pending'), 'class' => 'status-warning'],
    'in_progress' => ['label' => __('inquiry.in_progress'), 'class' => 'status-info'],
    'resolved' => ['label' => __('inquiry.resolved'), 'class' => 'status-success'],
    'closed' => ['label' => __('inquiry.closed'), 'class' => 'status-muted']
];

// Statistics
$totalInquiries = count($inquiries);
$pendingInquiries = count(array_filter($inquiries, function($i) { return ($i['status'] ?? 'pending') === 'pending'; }));
$inProgressInquiries = count(array_filter($inquiries, function($i) { return ($i['status'] ?? '') === 'in_progress'; }));
$resolvedInquiries = count(array_filter($inquiries, function($i) { return ($i['status'] ?? '') === 'resolved'; }));
?>
...
<div>
        ...
        <div class="card card-lg">
            <div class="flex-between-start mb-30">
                <div>
                    ...
                    <?php if (!empty($inquiryDetails['email'])): ?>
                        <p class="text-muted mb-5">Email: <strong><?php echo htmlspecialchars($inquiryDetails['email']); ?></strong></p>
                    <?php endif; ?>
                    
                    <?php if (!empty($inquiryDetails['phone'])): ?>
                        <p class="text-muted mb-5"> <?php echo __('inquiry.phone'); ?>: <strong><?php echo htmlspecialchars($inquiryDetails['phone']); ?></strong></p>
                    <?php endif; ?>
                    
                    <?php if (!empty($inquiryDetails['instagram'])): ?>
                        <p class="text-muted mb-5">Instagram: <strong><?php echo htmlspecialchars($inquiryDetails['instagram']); ?></strong></p>
                    <?php endif; ?>
    ...
    <?php else: ?>
        ...
        <div class="grid grid-auto-200 mb-30">
            ...
            <div class="card">
                <h4 class="text-sm text-muted mb-10"><?php echo icon_check_circle(16); ?> <?php echo __('inquiry.resolved'); ?></h4>
