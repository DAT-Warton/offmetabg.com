...
$orderStatuses = [
    'pending' => ['label' => __('order.pending'), 'class' => 'status-warning', 'icon' => '⌛'],
    'confirmed' => ['label' => __('order.confirmed'), 'class' => 'status-info', 'icon' => '✓'],
    'processing' => ['label' => __('order.processing'), 'class' => 'status-primary', 'icon' => '⚙️'],
    'shipped' => ['label' => __('order.shipped'), 'class' => 'status-info', 'icon' => '🚚'],
    'delivered' => ['label' => __('order.delivered'), 'class' => 'status-success', 'icon' => '<svg width="16"height="16"viewBox="0 0 24 24"fill="none"stroke="currentColor"stroke-width="2"><polyline points="20 6 9 17 4 12"></polyline></svg>'],
    'cancelled' => ['label' => __('order.cancelled'), 'class' => 'status-danger', 'icon' => '<svg width="16"height="16"viewBox="0 0 24 24"fill="none"stroke="currentColor"stroke-width="2"><line x1="18"y1="6"x2="6"y2="18"></line><line x1="6"y1="6"x2="18"y2="18"></line></svg>'],
];

// Statistics
$totalOrders = count($orders);
$pendingOrders = count(array_filter($orders, function($o) { return ($o['status'] ?? 'pending') === 'pending'; }));
$processingOrders = count(array_filter($orders, function($o) { return in_array($o['status'] ?? '', ['confirmed', 'processing', 'shipped']); }));
...
<div>
    ...
    <?php if ($orderDetails): ?>
        ...
        <div class="card card-lg">
            ...
            <form method="POST"class="card-muted mb-30">
                ...
                <div class="flex-end">
                    <div class="flex-1">
                        <label for="order_status"><?php echo __('admin.update_order_status'); ?>:</label>
                        <select id="order_status" name="status"class="select-plain select-block">
                ...
                <tbody>
                    <?php foreach ($orderDetails['items'] as $item): ?>
                        <tr>
                            ...
                            <td class="text-right"><?php echo $currency_symbol; ?><?php echo number_format($item['price'], 2); ?></td>
                            <td class="text-right font-semibold"><?php echo $currency_symbol; ?><?php echo number_format($item['price'] * $item['quantity'], 2); ?></td>
                        </tr>
                    <?php endforeach; ?>
                    <tr>
                        <td colspan="3"class="text-right font-semibold text-lg"><?php echo __('cart_page.total'); ?>:</td>
                        <td class="text-right font-bold text-lg text-primary"><?php echo $currency_symbol; ?><?php echo number_format($orderDetails['total'], 2); ?></td>
                    </tr>
                </tbody>
            </table>
    ...
    <?php else: ?>
        ...
        <div class="grid grid-auto-200 mb-30">
            ...
            <div class="card">
                <h4 class="text-sm text-muted mb-10"><?php echo icon_check_circle(16); ?> <?php echo __('order.completed'); ?></h4>
            ...
            <div class="card">
                ...
                <p class="stat-number text-success"><?php echo $currency_symbol; ?><?php echo number_format($totalRevenue, 2); ?></p>
            </div>
        </div>
                    ...
                    <tbody>
                        <?php foreach (array_reverse($orders, true) as $id => $order): ?>
                            ...
                            <tr class="table-row-hover">
                                ...
                                <td class="text-right font-semibold"><?php echo $currency_symbol; ?><?php echo number_format($order['total'], 2); ?></td>
