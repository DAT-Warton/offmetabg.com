...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_settings') {
    ...
    foreach ($_POST as $key => $value) {
        ...
        // Find the setting
        $stmt = $pdo->prepare("SELECT id, setting_type, is_encrypted FROM site_settings WHERE setting_key = ?");
        $stmt->execute([$key]);
        ...
        if ($setting) {
            ...
            // Update setting based on database driver
            $driver = $db->getDriver();
            if ($driver === 'pgsql') {
                $update_stmt = $pdo->prepare("UPDATE site_settings SET setting_value = ?, updated_at = NOW() WHERE id = ?");
            } else {
                $update_stmt = $pdo->prepare("UPDATE site_settings SET setting_value = ?, updated_at = NOW() WHERE id = ?");
            }
...
$stmt = $pdo->query("SELECT * FROM site_settings ORDER BY category, display_order, setting_key");
...
.setting-item select:focus {
    ...
    box-shadow: 0 0 0 2px var(--color-purple-alpha-20);
}
...
.btn-save:hover {
    ...
    box-shadow: 0 4px 12px var(--color-purple-alpha-30);
}
                        ...
                        switch ($setting['setting_type']):
                            ...
                            case 'select':
                                ...
                                <select name="<?php echo $name; ?>">
                                    <?php
                                    // Extract options based on setting key
                                    $options = [];
